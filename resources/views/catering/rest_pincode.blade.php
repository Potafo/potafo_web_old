...
@section('content')
    ...
    <div class="col-sm-12">
        ...
        <div class="col-sm-12">
            ...
            <a ><div class="potafo_top_menu_sec potafo_top_menu_act">Pincode</div></a>
            ...
            <div class="box master-add-field-box" >
                ...
                <div class="table-filter" style="margin-top: 4px;">
                  <div class="table-filter-cc">
                    <a onclick="enable_divs('add')"> <button type="submit" style="margin-left:0" class="on-default followups-popup-btn btn btn-primary " >Add New</button></a>
                </div>
               ...
               <div class=" pull-right">
                <div class="table-filter" style="margin-top: 4px;">
                  <div class="table-filter-cc">
                    <a title="Filter" href="#" onclick="enable_divs('filter')"> <button type="submit"  style="margin-right: 10px;" class="on-default followups-popup-btn btn btn-primary filter_sec_btn " ><i class="fa fa-filter" aria-hidden="true"></i></button></a>
                </div>
...
<div class="filter_box_section_cc_1  div_addpincode" style="display: none">
               <div class="filter_box_section">Add Pincodes</div>
                   <div class="filter_text_box_row">
                     <form  id="addpincode_form" role="form" method="POST" action="" >  
                       ...
                       <div class="main_inner_class_track" style="width: 25%;">
                          <div class="group">
                             <div style="position: relative">
                                 ...
                                 <!-- <input type="text" id="rest_pincode" name="rest_pincode"/>-->
                                  <select id="rest_cityid" name ="rest_cityid" class="form-control" onchange="loadpincodes(this.value)">
                       ...
                       <div class="main_inner_class_track loadpincode_div" style="width: 25%; display:none">
                          <div class="group">
                             <div style="position: relative">
                                  <label>Select Pincode</label>
                                 <!-- <input type="text" id="rest_pincode" name="rest_pincode"/>-->
                                  <div class="load_citypincodes"> </div>
                    ...
                    <div class="table-filter" style="margin-top: 4px;">
                        <div class="table-filter-cc">
                            <a style="cursor:pointer;" onclick="submitpincode({{$restaurant_id}})">submit</a>
 ...
 <div class="filter_box_section_cc  div_filterpincode" style="display: none">
                <div class="filter_box_section">Filter Pincodes</div>
                   <div class="filter_text_box_row">
                     <form  id="addpincode_form" role="form" method="POST" action="" >  
                       <input type="hidden" id="staff_id" name="staff_id" value="{{ Session::get('staffid')}}"/>
                       <input type="hidden" id="logingroup" name="logingroup" value="{{ Session::get('logingroup')}}"/>
                       ...
                       <div class="main_inner_class_track" style="width: 25%;">
                          <div class="group">
                             <div style="position: relative">
                                  <label>Select City</label>
                                 <!-- <input type="text" id="rest_pincode" name="rest_pincode"/>-->
                                  <select id="rest_cityid_filter" name ="rest_cityid_filter" class="form-control" onchange="filter_pincodes({{$restaurant_id}},'city',this.value)">
                                     <option value="">All</option>
                                    @foreach($rows as $item)
                       ...
                       <div class="main_inner_class_track loadpincode_div_1" style="width: 25%; ">
                          <div class="group">
                             <div style="position: relative">
                                  <label>Select Pincode</label>
                                  <input type="text" id="rest_pincode_filter" name="rest_pincode_filter" onkeyup="filter_pincodes({{$restaurant_id}},'pincode',this.value)"/>
                                  <!--<div class="load_citypincodes"> </div>-->
                    ...
                    @if(count($details)>0)
                        @foreach($details as $key=>$item)
                            <tr>
                                ...
                                <td style="min-width:130px !important;">{{ ($item->pincodes) }}</td>
                               
                                <td style="text-align: left;min-width:130px !important;">
                                    <a onclick="return pincodedel('{{ ($item->pincodes) }}','{{$restaurant_id}}')" class="btn button_table clear_edit" >
...
//                    alert(errorThrown);
       ...
       /* $(".followups-popup-btn").click(function(){
        ...
        function pincodedel(pincode,restid)
        {
            if(confirm('Are you sure to delete?'))
            {
                ...
                $.ajax({
                    method: "get",
                    url: "../api/cat_restpincode_delete/" + pincode + "/" + restid,
                    cache: false,
                    crossDomain: true,
                    async: false,
                    dataType: 'text',
...
function submitpincode(restid)
{
  var  rest_cityid = $("#rest_cityid").val(); 
  if(rest_cityid == '') 
            {
                $("#rest_city").focus();
                $.Notification.autoHideNotify('error', 'bottom right','Select City.');
                return false;
            }
    var  rest_pincode = $("#rest_pincode").val();  
  ...
  if(rest_pincode == '') 
            ...
            if(true)
            {
               // var formdata = new FormData($('#cat_restcategory')[0]);
                ...
                var i=1;
                var data= {"restid":restid,"rest_cityid":rest_cityid,"rest_pincode":rest_pincode};
                $.ajax({
                    method: "post",
                    url : "../api/add_restpincode",
                    data:data,
                    cache: false,
                    crossDomain: true,
                    async: false,
...
function loadpincodes(cityid)
    {
        ...
        $(".loadpincode_div").css("display",'block');
        $.ajax({
                    method: "get",
                    url : "../api/load_pincodes/" + cityid,
                    cache: false,
                    crossDomain: true,
                    async: false,
                    dataType: 'text',
    ...
    function enable_divs(type)
    {
        if(type=="add")
        {
          //$(".div_addpincode").css("display",'block'); 
         // $(".div_addpincode").toggleClass('diply_tgl');
          if($('.div_addpincode').css('display') == 'none')
                {
                   $(".div_addpincode").css("display",'block');
                }else
                {
                    $(".div_addpincode").css("display",'none');
                }
        }
        if(type=="filter")
        {
            if($('.div_filterpincode').css('display') == 'none')
                {
                   $(".div_filterpincode").css("display",'block');
                }else
                {
                    $(".div_filterpincode").css("display",'none');
                }
                
            
        }
    }
    function filter_pincodes(restid,type,keyvalue)
    {
        var cityid="";
        var pincode ="";
        if(type=="city")
         ...
         if(true)
            {
                ...
                var i=1;
                var data= {"restid":restid,"rest_cityid":cityid,"rest_pincode":pincode};
                $.ajax({
                    method: "post",
                    url : "../api/filter/cat_pincodes",
                    data:data,
                    cache: false,
                    crossDomain: true,
                    async: false,
                  ...
                  $.each(json_x, function (i, val)
                  {
                     ...
                     //alert(status)
                       var newRow = '<tr><td style="min-width:3px;">'+count+'</td>'+
                          '<td style="min-width:160px;text-align: left;">'+val.pincodes+'</td>'+
                          
                         
                          '<td  style="min-width:10px"> '+
                          '<a onclick="return pincodedel('+val.pincodes+','+restid+')" class="btn button_table clear_edit" >'+
