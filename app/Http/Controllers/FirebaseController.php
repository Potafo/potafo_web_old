...
public function fb_check_inprogress($area)
{
    $main_values = DB::SELECT("SELECT del_staff_auto_assign FROM `general_settings`");
	DB::select("DELETE FROM all_api_call_record  WHERE time(entry_date) < (SUBTIME(current_time(), '01:00:00'))");
	DB::select("INSERT INTO `all_api_call_record`(`entry_date`,area) VALUES (now(),$area)");
 
    $check_auto_assign=$main_values[0]->del_staff_auto_assign;
    if($check_auto_assign=="Y")
    {
		$this->fb_distance_staffid_check($area);
		
		/* $inprogress_check= DB::SELECT("SELECT * FROM order_master o,restaurant_master r WHERE  o.rest_id = r.id and o.assign_status = 'Inprogress'  AND o.current_status = 'P' and r.city = $area ");
 ...
 public function fb_distance_staffid_check($area1)
    {
    ...
    if($check_auto_assign=="Y")
    {
        /* $inprogress_check= DB::SELECT("SELECT * FROM order_master o,restaurant_master r WHERE  o.rest_id = r.id and o.assign_status = 'Inprogress'  AND o.current_status = 'P' and r.city = $area1");
        if(count($inprogress_check) > 0)
        {
           echo "Already Inprogress" ;
        }else
        { */           
            ...
            $main_values = DB::SELECT("SELECT o.order_number,o.rest_id,o.rest_confirmed_time,r.geo_cordinates,o.customer_details->'$.latitude' as latitude,o.customer_details->'$.longitude' as longitude FROM order_master as o JOIN restaurant_master as r WHERE r.id=o.rest_id  AND r.category != 'Potafo Mart' AND o.delivery_assigned_to is NULL AND (current_time() > o.assign_after_time)  AND o.assign_status = 'Pending'  AND o.current_status = 'P'  and r.city= $area1 order by o.assign_after_time asc LIMIT 1");
            if(count($main_values) > 0)
            {
               ...
               $staff_ordercount='';
               //$staff_ordercount='';
               $fb_stafflist = DB::SELECT("SELECT ds.staff_id as id,s.current_confirmed_count as confirmed_count FROM delivery_staff_attendence ds,internal_staffs s,internal_staffs_area sa WHERE  ds.staff_id = s.id and s.id = sa.staff_id and sa.area_id = $area1 and s.order_list_cat != 'potafo_mart' and s.assing_status = 'Available' AND (current_time() > s.auto_assign_after) and out_time IS NULL ORDER BY ds.staff_id ASC");
               if(count($fb_stafflist)>0)
                 {
                    foreach($fb_stafflist as $key=>$items)
                    {
                        $list_staff_id = $items->id;
                        $staff_current_order_count = DB::SELECT("SELECT count(order_number) as current_count FROM order_master WHERE delivery_assigned_to = '$list_staff_id' AND current_status in ('C','OP')");
                        $order_val = $staff_current_order_count[0]->current_count;

                        //$order_val=$items->confirmed_count;
                                ...
                                if($finalstaff==0)
                                {
                                    for($i=1;$i<=$assign_inc_loop_count;$i++)
                                    {
                                        if($loopstatus=="sorry")
                                        {
                                            ...
                                            if($finalstaff!=0)
                                            {
                                                $stafcrdt=$this->check_staff_credits($finalstaff,$order_number);
                                                if($stafcrdt=="Valid")
                                                {
                            ...
                            if($loopstatus=="sorry") 
                               {
                                if($finalstaff!=0)
                                {
                                   ...
                                   DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='1' WHERE `id`='".$finalstaff."'  and`assing_order_no`='".$order_number."'");
                                   //print_r($staff_ordercount[0]);
                                   //echo count($staff_ordercount[0]);
                                   if((count($staff_ordercount[0]))==0)
                                   {
                             ...
                             if($loopstatus=="sorry") 
                               {
                                 if($finalstaff!=0)
                                {
                                   ...
                                   DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='2' WHERE `id`='".$finalstaff."'  and`assing_order_no`='".$order_number."' ");
                                   if((count($staff_ordercount[1]))==0)
                                   {
                 ...
                 if($loopstatus=="sorry")
                            {
                             ...
                             DB::UPDATE('update order_master set assign_status = "Pending"  where order_number = "'.$order_number.'"');   
                            }
                DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='3' WHERE `assing_order_no`='".$order_number."' ");            
                            
                } catch (\Exception $e) {

                echo $e->getMessage();
                 DB::UPDATE('update order_master set assign_status = "Pending"  where order_number = "'.$order_number.'"');
                 DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='4' WHERE `assing_order_no`='".$order_number."'");            
                 $timezone = 'ASIA/KOLKATA';
                $date = new DateTime('now', new DateTimeZone($timezone));
                $datetime = $date->format('Y-m-d H:i:s');
                 DB::SELECT('INSERT INTO `all_exceptions`(`entry_date`, `error_section`, `error_message`) VALUES("'.$datetime.'","FirebaseController-fb_distance_staffid_check","'.$e->getMessage().'") ');
                }
    ...
    {
        $inprogress_check= DB::SELECT("SELECT order_number FROM order_master WHERE  assign_status = 'Inprogress' AND order_number = '".$order_number."' AND current_status = 'P'");
        if(count($inprogress_check) > 0)
        {
            $timezone = 'ASIA/KOLKATA';
            $date = new DateTime('now', new DateTimeZone($timezone));
            ...
            $staff_attendance= DB::SELECT("SELECT ds.in_time FROM delivery_staff_attendence ds WHERE ds.staff_id  = '".$staffid."' and ds.entry_date = current_date() and ds.out_time is null");
            if(count($staff_attendance) < 1)
            {
                DB::UPDATE('update order_master set assign_status = "Pending"  where order_number = "'.$order_number.'"');
                DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='13' WHERE  `id`='".$staffid."'  and `assing_order_no`='".$order_number."' ");
                return "sorry";
            }
            $exist = DB::SELECT("SELECT ftoken,staff_max_credit,first_name,mobile,auto_assign_next_order_2 from internal_staffs where id=$staffid");
            $staff_name=$exist[0]->first_name;
            $staff_number=$exist[0]->mobile;
            $staff_current_order_count = DB::SELECT("SELECT count(order_number) as current_count FROM order_master WHERE delivery_assigned_to = '$staffid' AND current_status in ('C','OP')");
            $order_val = $staff_current_order_count[0]->current_count;
            $max_order_loop = 2;
            if($order_val >= $max_order_loop)
            {
                DB::UPDATE('update order_master set assign_status = "Pending"  where order_number = "'.$order_number.'"');
                DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='11' WHERE  `id`='".$staffid."'  and `assing_order_no`='".$order_number."' ");
                return "sorry";
            }
            if($order_val == 1)
            {
                $auto_assign_next_order_2 = $exist[0]->auto_assign_next_order_2;
                if($order_number != $auto_assign_next_order_2)
                {
                    DB::UPDATE('update order_master set assign_status = "Pending"  where order_number = "'.$order_number.'"');
                    DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='12' WHERE  `id`='".$staffid."'  and `assing_order_no`='".$order_number."' ");
                    return "sorry";
                }
            }
            ...
            $cst_address1 = $contact[0]->cst_address1;
            ...
            if(count($is_exist)>0)
            {
                foreach($is_exist as $item)
                {
                    $arr['to'] = $item->ftoken;
                    $arr['title'] = 'Order Assigned';
                    $arr['message'] = 'Your Order from '.$hotel_name.' ,Order Number- '.$order_number.' has been assigned to our Delivery staff - ' . $staff_name . ' for picking.';
                    $arr['image'] = 'null';
                    $arr['action'] = 'orderhistory';
                    $arr['action_destination'] = 'null';
            ...
            DB::SELECT("UPDATE internal_staffs SET current_confirmed_count = $order_val, dlv_area = '$cst_address1' where id = $staffid");
            ...
            DB::SELECT("INSERT INTO `internal_staffs_current_orders`(`isco_staff_id`, `isco_slno`, `isco_order_no`, `isco_assigned_time`, `isco_cust_lat`, `isco_cust_long`) VALUES ('".$staffid."','0','".$order_number."','".$datetime."','".$restlist_data['cust_lat']."','".$restlist_data['cust_long']."')");
            ...
            DB::UPDATE('UPDATE internal_staffs SET auto_assign_next_order_2 = NULL WHERE  auto_assign_next_order_2 = "'.$order_number.'"');
    ...
    {
         $rowkey = DB::SELECT("SELECT `assign_min_dist_meter`,`assign_inc_dist_meter`,assign_inc_loop_count,`assign_max_order`,`assign_max_order_time_gap`,`assign_nxt_rest_max_dist`,`assign_nxt_cust_max_dist`,`assign_nxt_after_pick` FROM `general_settings`");
         $assign_max_order_time_gap = $rowkey[0]->assign_max_order_time_gap;
         $assign_nxt_rest_max_dist = $rowkey[0]->assign_nxt_rest_max_dist;
         $assign_nxt_cust_max_dist = $rowkey[0]->assign_nxt_cust_max_dist;
         $assign_nxt_after_pick = $rowkey[0]->assign_nxt_after_pick;
        $database = $this->firebase_db; 
...
//        $cust_long_old= $orderdet[$orderid]['cust_long'];
        $stafdetls = DB::SELECT("SELECT  `isco_order_no`,`isco_cust_lat`,`isco_cust_long` FROM `internal_staffs_current_orders` WHERE `isco_staff_id`='".$finalstaff."' order by `isco_assigned_time` DESC LIMIT 1");
        if(count($stafdetls) != 0)
        {
             $orderid =$stafdetls[0]->isco_order_no;
             $cust_lat_old= $stafdetls[0]->isco_cust_lat;
             $cust_long_old= $stafdetls[0]->isco_cust_long;


            $old_order_number = DB::SELECT("SELECT o.rest_confirmed_time,r.geo_cordinates,o.current_status FROM order_master as o JOIN restaurant_master as r WHERE r.id=o.rest_id AND o.order_number='".$orderid."'");
            $rest_confirmed_time_old_order=$old_order_number[0]->rest_confirmed_time;
            $rest_det=explode(",",$old_order_number[0]->geo_cordinates);
            $current_status=$old_order_number[0]->current_status;
            $rest_lat_old=$rest_det[0];
            $rest_long_old=$rest_det[1];
            if($current_status != "C" && $assign_nxt_after_pick == "N")
            {
                DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='5' WHERE `id`='".$finalstaff."'  and`assing_order_no`='".$order_number."'");
                return "sorry";
            }
           $time_diff=time_difference($rest_confirmed_time_old_order,$rest_confirmed_time);
            if(strtotime($time_diff) <= strtotime($assign_max_order_time_gap))
            {
                ...
                $rest_distance_mtr= ($rest_distance * 1000 );
                if($rest_distance_mtr <= $assign_nxt_rest_max_dist)
                {
                     $cust_distance = Commonsource::distance_calculate($cust_lat_old,$cust_lat,$cust_long_old,$cust_long);
                     $cust_distance_mtr= ($cust_distance * 1000 );
                    if($cust_distance_mtr <= $assign_nxt_cust_max_dist)
                    {
                        DB::SELECT("UPDATE internal_staffs SET auto_assign_next_order_2 ='".$order_number."' WHERE id ='".$finalstaff."' AND  auto_assign_next_order_2 is NULL");
                        $this->updatetofirebase($finalstaff, $order_number); //break; 
                        return "finished";
                    }else
                    {
                        DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='6' WHERE `id`='".$finalstaff."' and`assing_order_no`='".$order_number."'");
                        return "sorry";
                        //break;
                    }
                }else
                {
                    DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='7' WHERE `id`='".$finalstaff."'  and`assing_order_no`='".$order_number."'");
                    return "sorry";
                    //break;
                }
            }else
            {
                DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='8' WHERE `id`='".$finalstaff."'  and`assing_order_no`='".$order_number."'");
                return "sorry";
                //break;
            }
        }else
        {
            DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Available',assing_order_no='9' WHERE `id`='".$finalstaff."'  and`assing_order_no`='".$order_number."'");
            return "sorry";
            //break;
        }
    }

    public function check_staff_credits($staffid,$order_number)
    {
        $staffamount = DB::SELECT("select a.staff_credit - b.total  as pending_amount from ( select staff_max_credit as staff_credit,id from  `internal_staffs` where id = '".$staffid."') as a Join (select sum(final_total) as total,staff_id from `internal_staffs_credits` where status in ('Reserve','Credit') and staff_id = '".$staffid."') as b on a.id = b.staff_id");
        if(count($staffamount) != 0)
        {
        ...
        {//SELECT * FROM `internal_staffs` WHERE `id`='' AND `assing_status`
            $check_staff = DB::SELECT("SELECT assing_status FROM `internal_staffs` WHERE `id`='".$staffid."' AND `assing_status`='Available'");
            if(count($check_staff) != 0)
            {
               DB::SELECT("UPDATE `internal_staffs` SET `assing_status`='Ongoing',assing_order_no='".$order_number."' WHERE `id`='".$staffid."'"); 
               return "Valid";
            }else
            {
                return "Invalid";
