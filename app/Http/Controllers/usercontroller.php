...
$get_report = DB::SELECT("SELECT DISTINCT b.rm_name, c.ru_access, c.ru_id FROM tbl_report_category a LEFT JOIN tbl_report_master b ON b.rm_main_cat = a.rc_id LEFT JOIN tbl_report_users c ON c.ru_id = b.rm_id WHERE b.rm_active='Y' AND c.ru_userid='".$userid."' $condition");
    ...
    public function generalsettings(Request $request)
    {
         ...
         if($details == 'Exist') {
              $restaurantsettings = DB::SELECT('select r.force_close,r.busy,uc.login_group as close_by,ub.login_group as busy_by from restaurant_master r left join users uc on  r.forceclose_by = uc.id left join users ub on  r.busy_by = ub.id where r.id = "'.$restaurantid.'"');
              $general= GeneralSetting::where('id','1')->select('restaurant_time_count','restaurant_from_time','restaurant_to_time')->first();
              ...
              $rstsettings = DB::SELECT('select area_helpdesk from `restaurant_master` as rst join `city`as cty on rst.city =  cty.id where rst.id = "'.$restaurantid.'"');
              $usertypecheck = DB::SELECT('SELECT `login_group` FROM `users` WHERE `id` ="'.$userid.'"');
			  $msg = 'Exist';
			  
			  if(($restaurantsettings[0]->close_by)==($usertypecheck[0]->login_group))
			  {
				  $force_change ='Y';
			  }
			  else
			  ...
			  if(($restaurantsettings[0]->busy_by)==($usertypecheck[0]->login_group))
			  {
				  $busy_change ='Y';
			  }
			  else
			  ...
			  if(($restaurantsettings[0]->force_close)== 'N' )
			  {
				  $force_change ='Y';
			  }
			  if(($restaurantsettings[0]->busy)== 'N' )
			  {
				  $busy_change ='Y';
			  } 
			  ...
			  return response::json(['msg'=>$msg,
                      ...
                      'area_helpdesk'=>$rstsettings[0]->area_helpdesk,
                      'restaurant_time_count'=> $general['restaurant_time_count'],'timings'=>$day,
                      'potafo_from_time' =>$general['restaurant_from_time'],
                      'potafo_to_time' =>$general['restaurant_to_time'],
                      ]);
          }
          else
          {
    ...
    public function restauranttimebyday(Request $request)
    {
        ...
        if($details == 'Exist')
        {
            $timings = DB::SELECT('SELECT rt_day,rt_from_time,rt_to_time,rt_slno FROM `restaurant_timings` where rt_rest_id = "'.$restaurantid.'" and rt_day = "'.$day.'"');
            if(count($timings) > 0)
            {
                $msg ='Exist';
                return response::json(compact('msg','timings'));
    ...
    public function busyupdate(Request $request)
    {
        ...
        if($details == 'Exist') {
            ...
            {
              $msg = 'Success';
			  $restaurantsettings = DB::SELECT('select r.busy from restaurant_master r  where r.id = "'.$restaurantid.'"');
			  if(($restaurantsettings[0]->busy)!= $status )
			  {
    ...
    public function forcecloseupdate(Request $request)
    {
        ...
        if($details == 'Exist') {
            ...
            if(count($restaurant)>0)
            {
				...
				$restaurantsettings = DB::SELECT('select r.force_close from restaurant_master r  where r.id = "'.$restaurantid.'"');
			  if(($restaurantsettings[0]->force_close)!= $status )
			  {
				     Restaurant_Master::where('id', $restaurantid)->update(
                    [
